...
func TestBMrayRealityXray(t *testing.T) {
	...
	if binary == "" {
		t.Skip("set BMRAY_TEST_XRAY to the pinned Xray binary")
	}
	...
	cmd := exec.Command(binary, "run", "-c", filepath.Join(dir, "server.json"))
	cmd.Stdout = logFile
	cmd.Stderr = logFile
	...
	for _, fp := range []string{"firefox", "chrome", "qq"} {
		for _, invalid := range []string{"valid", "key", "short_id"} {
			...
			t.Run(fp+"/"+invalid, func(t *testing.T) {
				publicKey := encode(key.PublicKey().Bytes())
				shortID := sid
				if invalid == "key" {
					publicKey = encode(wrongKey.PublicKey().Bytes())
				}
				if invalid == "short_id" {
				...
				raw, e := (&net.Dialer{}).DialContext(ctx, "tcp", address)
				if e != nil {
					t.Fatal(e)
				}
				defer raw.Close()
				...
				if invalid != "valid" || legacy {
					if e == nil {
						secured.Close()
						t.Fatal("unauthenticated/legacy handshake was accepted")
					}
					t.Log("expected rejection:", e)
					return
				}
				if e != nil {
					t.Fatal("REALITY handshake:", e)
				}
				client, e := vless.NewClient(uuid, "xtls-rprx-vision", logger.NOP())
				if e != nil {
					t.Fatal(e)
				}
				conn, e := client.DialEarlyConn(secured, M.ParseSocksaddr(destination.Listener.Addr().String()))
				if e != nil {
					t.Fatal(e)
				}
				defer conn.Close()
				...
				response, e := (&http.Client{Transport: transport}).Do(request)
				if e != nil {
					t.Fatal("HTTPS over VLESS Vision:", e)
				}
				...
				if response.StatusCode != 200 || string(body) != payload {
